...
teaser: Inject failure modes into the Compliance handler, run a lifecycle starter that exercises each one, and observe non-retryable errors, retryable backoff, cancellation propagation, and circuit-breaker open state in the UI and CLI.
notes:
- type: text
  contents: |-
