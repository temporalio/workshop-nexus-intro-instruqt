...
- Runs **inline** in the handler Worker's process. **No workflow** on the implementer side.
...
- **Build 2** Runs inline in the handler Worker's process. No workflow on the implementer side.
...
- **Build 4** Mental model: a function call across a namespace boundary.
  - They already know how to call functions and Activities.
...
- **Build 4 (line 5, sync_operation decorator)** `@nexusrpc.handler.sync_operation`
  - The per-method decorator that says "run inline, return synchronously, finish in ≤10s."
...
<!--
- A sync handler runs inline on the **handler's** Worker, bound by a 10-second per-request deadline measured by the caller's Nexus Machinery.
  - The deadline is request-level, measured by the caller's Nexus Machinery, not derived from Workflow Task runtime.
  - The handler runs on the implementer-side Worker that polled the Nexus Task. The caller's Workflow Task already closed when it issued the `ScheduleNexusOperation` command; a new caller-side Workflow Task is scheduled later to deliver the result.
...
- **Build 6** Most production sync timeouts in the wild are handler workers not running.
  - Mystery 10s timeouts are almost always "the worker pool scaled to zero."
...
- AhaSlides multi-select: "When is a sync Nexus handler the right tool?"
  ...
  - The Signal/Query/Update answer is the canonical sync use case per the docs and is exactly what `submit_review` is going to do later in the workshop.
- AhaSlides pick answer: "Your sync handler routinely takes 9.8s. What's the safe move?"
...
- Endpoints are global per Temporal cluster, not per-namespace. They're a shared resource on Temporal Cloud.
-->
...
- **Build 1** Two namespaces, two Workers, one Endpoint
  - Two boxes for namespaces, two Workers, one routing icon.
